...
class Caseolap(object):
    ...
    def __init__(self, category2pmids, pmid2entity2count, result_dir, 
                 categorys_path, logfile):
        
        self.category_names = json.load(open(categorys_path))
    ...
    def print_categories2pmid(self, dump = False, verbose =False):
        '''
        FUNCTION:
        ...
        # Print counts: header
        if verbose:
            self.print_progress('Category: # PMIDs collected\n'+'='*27)
        ...
        # Export counts
        if dump:
            self.dump_json(self.category2pmids, file_name = 'categpmids')
    ...
    def get_all_entities(self, dump = False,verbose = False):
        ...
        # Export
        if dump:
            self.dump_json(self.all_entities, file_name = 'all_entities')
            self.dump_json(self.category2entities, file_name = 'category2entities')
    ...
    def category2entity2tf_finder(self): 
        ...
        for category, entity2nonzero_count in self.category2entity2count.items():
            ...
            for entity in self.all_entities:
                ...
                # Count
                try: count = entity2nonzero_count[entity]
                except: count = 0
    ...
    def calculate_all_popularity_scores(self, dump = False):
        '''
        FUNCTION:
        ...
        if dump:
            self.df_builder(self.category2entity2popularity, 'popularity_score')
    ...
    def calculate_all_distinctiveness_scores(self, dump = False): 
        '''
        FUNCTION:
        ...
        # Export
        if dump:
            self.df_builder(self.category2entity2distinctiveness,
                            'distinctiveness_score')
            
            
            
            
    def calculate_caseolap_score(self, caseolap_name, dump=False):
        '''
        FUNCTION:
        ...
        # Export
        if dump:
            self.df_builder(self.category2entity2caseolap, caseolap_name)
            self.dump_json(self.category2entity2caseolap, caseolap_name)
            self.dump_json(self.result_stat, 'result_stat')
