import os
os.system('cp ../biomedkg_utils.py .')
import json
from biomedkg_utils import *



def load_protein2pathway_data(uniprot2reactome_file_path):
    ...
    json.dump(protein2pathway, open('data/protein2pathway','w'))
    json.dump(pathway2protein, open('data/pathway2protein','w'))
    
    return protein2pathway, pathway2protein


def load_protein2pathway_data_PE(uniprot2reactome_file_path):
...
def map_protein2go_ids():
    relations, go_terms, total = dict(), set(), 0
    protein2go = dict()
    go2protein = dict()

    for i, line in enumerate(open('data/goa_human.gaf')):
        if i > 40:
            line = line.split('\t')
            protein = line[1]
            relation = line[3]
    ...
    json.dump(protein2go, open('data/protein2go.json','w'))
    json.dump(go2protein, open('data/go2protein.json','w'))
...
def get_go_term_information():
    # Convert GO obo file to dict
    ID = ''
    go_dict = dict()
    for line in open('data/go-basic.obo'):
